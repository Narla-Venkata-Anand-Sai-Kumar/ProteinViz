...
class view(object):
    ...
    def __init__(self,query='',width=640,height=480,viewergrid=None,data=None,style=None,linked=True,options=dict(),js='https://cdnjs.cloudflare.com/ajax/libs/3Dmol/2.0.4/3Dmol-min.js'):
        ...
        if type(height) == int:
            height = '%dpx'%height
        self.startjs = '''<div id="%s"  style="position: absolute; width: %s ; height: %s ;">
...
""" % (js,warnid)
        ...
        if viewergrid: #create split view
            ...
            self.startjs += 'viewergrid_UNIQUEID = $3Dmol.createViewerGrid(document.getElementById("%s"),{rows: %d, cols: %d, control_all: %s},{backgroundColor:#FFFFFF00});\n' % (divid, viewergrid[0],viewergrid[1],'true' if linked else 'false')
            self.startjs += "viewer_UNIQUEID = viewergrid_UNIQUEID[0][0];\n"
            self.viewergrid = viewergrid
        else:
            self.startjs += 'viewer_UNIQUEID = $3Dmol.createViewer(document.getElementById("%s"));\nviewer_NVANANDSAIKUMAR.setBackgroundColor("#4577EB");\n' % divid
        if query:
            if viewergrid:
